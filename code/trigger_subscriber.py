...
class TrainingTriggerSubscriber:
    ...
    async def connect_and_subscribe(self):
        ...
        # Initialize reconnect backoff
        base_delay = self.reconnect_delay
        max_delay = 60  # Maximum delay between retries (seconds)
        current_delay = base_delay
        max_retries = 10  # Maximum number of consecutive retries
        retry_count = 0

        while self.should_reconnect:
            try:
                # Connect to the server with explicit connection timeout
                connection_timeout = 30  # seconds
                logger.info(f"Attempting connection with {connection_timeout}s timeout...")

                # Test TCP connection first to fail fast if server is unreachable
                try:
                    # Create a socket connection to test basic connectivity
                    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    sock.settimeout(5)  # 5 second timeout for TCP test

                    # Extract host and port from server_uri
                    ...
                    if result != 0:
                        raise ConnectionRefusedError(f"TCP connection to {host}:{port} failed with code {result}")

                    logger.info(f"✅ TCP connection to {host}:{port} successful, proceeding with WebSocket handshake")
                except Exception as e:
                    logger.error(f"❌ TCP connection test failed: {str(e)}")
                    raise ConnectionRefusedError(f"Server unreachable: {str(e)}")

                # Use ping_interval=None during initial connection to avoid handshake issues
                self.websocket = await asyncio.wait_for(
                    websockets.connect(
                        self.server_uri,
                        ping_interval=None,  # Disable ping during handshake
                        subprotocols=["websocket"],  # Explicitly specify protocol
                        max_size=10_485_760,  # 10MB max message size
                        compression=None,  # Disable compression for compatibility
                        open_timeout=connection_timeout  # Explicit open timeout
                    ),
                    timeout=connection_timeout
                )

                # After connection, set up ping for keeping connection alive
                if hasattr(self.websocket, "ping_interval") and self.websocket.ping_interval is None:
                    # Only set these if we need to and if the connection supports it
                    self.websocket.ping_interval = 20
                ...
                if retry_count >= max_retries:
                    logger.error(f"❌ Maximum retry attempts ({max_retries}) reached. Stopping reconnection attempts.")
                    self.should_reconnect = False
                    break

                if self.should_reconnect:
                    # Implement exponential backoff with jitter
                    jitter = random.uniform(0, 0.1 * current_delay)  # Add up to 10% jitter
                    delay = current_delay + jitter

                    logger.info(f"🔄 Retrying connection in {delay:.1f} seconds... (Attempt {retry_count}/{max_retries})")
                ...
                logger.error(f"❌ Connection timed out after {connection_timeout} seconds")
                self.is_connected = False
                retry_count += 1
                ...
                if self.should_reconnect:
                    # Use exponential backoff for timeouts too
                    logger.info(f"🔄 Retrying connection in {current_delay:.1f} seconds... (Attempt {retry_count}/{max_retries})")
